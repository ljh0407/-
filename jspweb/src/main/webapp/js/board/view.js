...
function bview(){
	$.ajax({
		...
		success : function( re ){ 
			...
			// 상대경로 : 현재 위치 기준 [ http://localhost:8080/jspweb/jsview.js ]
						// ../board/filedown
			/* 
				<a href= "URL?변수명=데이터">	: get 메소드로 해당 URL 데이터 전송가능
				'<a href= "URL?변수명='+변수명+'">'
			*/			
			console.log(board.bfile)
			
			if( board.bfile !== null ){ // null , undefined , 0 , flase
