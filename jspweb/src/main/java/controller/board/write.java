...
public class write extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * String mid = (String)request.getSession().getAttribute("mid"); // 1. 로그인 세션
		 * [로그인한 사람의 아이디] 가져오기 int mno = MemberDao.getInstance().getMno(mid); // 2.
		 * 회원아이디 ---> 회원번호 // fk는 pk에 있는 데이터만 저장가능 [무결성] String btitle =
		 * request.getParameter("btitle"); // 3. 입력받은 데이터1 요청 String bcontent =
		 * request.getParameter("bcontent"); // 3. 입력받은 데이터2 요청 System.out.println(
		 * btitle ); System.out.println( bcontent ); boolean result =
		 * BoardDao.getInstance().write(btitle, bcontent , mno ); // 4. Dao 호출 [ 저장 ]
		 * System.out.println( result ); js 전송용 response.getWriter().print(result); //
		 * 5. 결과를 AJAX 돌려주기
		 * 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// MultipartRequest : 첨부파일 지원o [ 대용량의 문자 가능 ]
		...
		// * 어떤파일을 업로드 했는지 db에 저장할 첨부파일 경로/이름
