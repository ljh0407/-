...
public class filedown extends HttpServlet {
	...
	//	String uploadpath = "C:\\Users\\504\\git\\-\\jspweb\\src\\main\\webapp\\upload\\"+bfile;
		...
		// 4. HTTP 해서 지원하는 다운로드 메소드 [ response ]
		response.setHeader( "Content-Disposition" , // 다운로드형식 HTML에서 지원 [ 브라우저 차이 있음 ]
							...
							// URL 경로에 한글이 있을경우 : URLEncoder.encode( 데이터 , "인코딩타입" )
		// 5. 파일 전송 [ 외부와 데이터 통신(스트림) = 바이트단위 ]
			// 1. [ 입력스트림 객체 생성 ]파일의 내용물의 바이트로 모두 읽어온다.
		...
		/*
		  	...
		  	업로드된폴더[윈도우]	------스트림	------>		자바			-----getOutputStream------> HTTP[웹]
		  
		  		파일			------input------->		바이트 배열		-----------output---------> 
		  					// 1. BufferedInputStream 클래스		* 실제 내용물 바이트 변환 			2. BufferedOutputStream 클래스
							// new BufferedInputStream( new FileInputStream(file));			new BufferedOutputStream(response.getOutputStream() );
