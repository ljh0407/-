		...
		while(true) { // 2. 무한루프 [ 무한반복 ]
			...
			if ( 메뉴선택 == 1 ) {		
				...
				if( 콜라재고 > 0 ) {	// 재고 있다
					...
					System.err.println("안내) 콜라재고가 없습니다. [죄송합니다] ");
				}
			} 		// 1 를 입력했을때
				...
				if( 환타재고 > 0 ) {	// 재고 있다
					...
					System.err.println("안내) 환타재고가 없습니다. [죄송합니다] ");
				}
			}	
				...
				if( 사이다재고 > 0 ) {	// 재고 있다
					...
					System.err.println("안내) 사이다재고가 없습니다. [죄송합니다] ");
				}
			}	
				...
				System.out.println("안내) 결제 하시겠습니까? [Y] [N] "); String 결제여부 = scanner.next();
				
				boolean 결제성공 = false; // 결제 성공 여부를 저장하는 변수 [ 스위치 변수 ]
			...
			else {System.out.println("안내) 선택할수 없는 번호입니다[다시선택] ");}						// 그외 입력했을때
		}
...
/* 
	자판기 프로그램
		...
		3. 메뉴판에서 선택한 제품을 장바구니 담기
			...
			2. 제품의 재고를 1차감 	-> 장바구니제품의 1증가
			3. '장바구니에 담았습니다.'
		4. 결제를 선택했을때 장바구니 현황 표시 결제
		5. 결제예정인 금액보다 투입부족 이 부족하면 결제취소 금액이 초과 잔돈 출력 
		[화면구현]
