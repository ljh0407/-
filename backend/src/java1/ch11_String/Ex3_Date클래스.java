...
public class Ex3_Date클래스 {
	...
	public static void main(String[] args) {
		...
		SimpleDateFormat adf = new SimpleDateFormat("yyyy년 MM월 dd일 hh시 mm분 ss초");
		String strNow2 = adf.format( new Date() );
		System.out.println(strNow2);
		
		// 3. Calendar 클래스
		Calendar cal = Calendar.getInstance(); // 운영체제에서 설정되어 있는 시간대날짜와 시간의 정보 호출
			// 객체명.get( 해당 필드명 )
				// 번호 : calender.필드 =>  Calendar.YEAR -> 1
		int year = cal.get( Calendar.YEAR); 		System.out.println(year);
		int month = cal.get( Calendar.MARCH) + 1; 	System.out.println(month); 
