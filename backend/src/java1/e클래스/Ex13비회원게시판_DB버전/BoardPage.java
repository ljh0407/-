...
public class BoardPage {
	...
	void view() {
		...
		while(true) { // 무한반복
			page.getBoardlist(); // 모든 게시물 출력 함수호출
			System.out.print("1.글쓰기 2.글제목 선택 : "); // 1. 안내[출력-print] -> 입력[scanner] -> 저장[변수]
			int btn = scanner.nextInt();// int 변수를 쓰는 이유??
			// 경우의수 2개+1(그외)			
			if( btn == 1 ) {page.regist();}			// 1번 입력하면 등록함수 호출
			else if ( btn == 2 ) {page.getBoard();}		// 2번 입력하면 등록함수 호출
	...
	void regist() {
		...
		System.out.print(">>비밀번호 :");String b_password = scanner.next();
		scanner.nextLine(); // nextLine() 겹치는 문제 해결방안 
		
		//2. 입력받은 데이터를 control로 이동[ 다른 클래스 메소드 호출 ]
		boolean result = control.regist(b_title, b_content, b_writer, b_password);
		// 3. control 처리된 결과를 result 에 저장한 다음에 제어 [ control 실행후 결과의 경우의수 ]
	...
	void getBoardlist() {
		...
		BoardDto[] boardlist = control.getBoardlist(); // control에 모든 게시물호출 메소드 호출
	...
	void getBoard() {
		...
		BoardPage page = new BoardPage();
		if( btn == 1 ) { return; }
		else if( btn == 2 ) { page.update();}  // 2번 선택시 업데이트 함수호출
		else if( btn == 3 ) { page.delete();}  // 3번 선택시 삭제 함수호출
		else {}
