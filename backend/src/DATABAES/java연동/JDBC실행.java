...
public class JDBC실행 {
	...
	public static void main(String[] args) {
		...
		// JDBC : 자바와 DB 연동
			...
			// MYSQL 연동 할때 해당 클래스에서 예외(오류발생)
				// * 연동 실패 했을 경우 그에 대한 대처 코드
				// 해결방안 : 예외처리 [ try ~ catch ]
		...
		/*
			...
			// 박진영
			sql = "intsert inot member values('jyp','박진영','경기 고양시 장항동')";
			ps = con.prepareStatement(sql);
			ps.executeUpdate();
			*/
		/*
			...
			*/
			...
			while( rs.next() ) { // while (조건) : 조건이 true 이면 반복문 실행
				// rs.next() : 다음 레코드로 이동
				System.out.print("회원아이디 : " + rs.getString(1) +"\t");
