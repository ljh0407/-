 ...
 */
 ...
 b.innerHTML = '<h1> html 형식으로 넣기</h1>'
	...
	// 2. p.315 속성 조작
const rects = document.querySelectorAll('.rect') // 해당 class의 모든요소 가져오기
	
		// 길이 1~10     	인덱스 0~9	
	for( let i = 0 ; i<rects.length ; i++){
		// [i : 인덱스로 사용할 변수] i는 0부터 마지막 인덱스 1씩 증가반복
		const width = ( i +1 )*100 
		const src = `http://placekitten.com/${width}/250`
		// const src =`../img/사진명 확장자`
		rects[i].setAttribute('src', src)
		// rects.src = src
		// 배열내 해당 i번째 인덱스와 태그에 속성을 추가 [ 이미지 경로 ]
	}
	// 3. p.318 스타일 조작
	const divs = document.querySelectorAll('#divbox div')
	
	divs.forEach( (div,index) => {
		// 배열명.forEach( [콜백함수] (요소명 , 인덱스 , 배열명 )=>{ } )
